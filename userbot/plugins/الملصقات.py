...
def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"rekhsobot_{userid}_{pack}_anim"
    return f"rekhsobot_{userid}_{pack}"
...
async def add_to_pack(    catevent,    conv,    args,    packname,    pack,    userid,    username,    is_anim,    stfile,    emoji,    cmd,    pkang=False ):
    ...
    return pack, packname
@rekhso.on(admin_cmd(pattern="صنع ملصق(?:\s|$)([\s\S]*)"))    
async def kang(args):  
    photo = None
    emojibypass = False
    is_anim = False
...
@rekhso.on(admin_cmd(pattern="جلب الملصقات(?:\s|$)([\s\S]*)"))    
...
@rekhso.on(admin_cmd(pattern="انشاء حزمه ملصقات(?:\s|$)([\s\S]*)"))   
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "rekhsobot_" + "".join(        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)    )
...
@rekhso.on(admin_cmd(pattern="جلب معلومات الملصق$"))    
...
@rekhso.on(admin_cmd(pattern="ملصق ?([\s\S]*)"))    
