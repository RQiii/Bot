...
@rekhso.iq_cmd(pattern="تكرار حزمه الملصقات$",)
...
@rekhso.iq_cmd(    pattern="تكرار_احرف ([\s\S]*)",)
...
@rekhso.iq_cmd(    pattern="تكرار_كلمه ([\s\S]*)",)
async def tmeme(event):
...
@rekhso.iq_cmd(pattern=f"{MUQT} ([\s\S]*)",)
async def spammer(event):
    ...
    try:
        ...
        return await edit_delete(            event, "عذرا طريقه كتابة الأمر خطأ - شرح الارسال الوقتي او مؤقت للكروبات هنا : https://t.me/pmmvn/4483"        )
    await event.delete()
    addgvar("spamwork", True)
