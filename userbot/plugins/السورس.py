...
from userbot import StartTime, rekhso, catversion
...
rekhsoPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/c6a84d3209402e5cd422d.mp4"
...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/c6a84d3209402e5cd422d.mp4"
...
@rekhso.on(admin_cmd(pattern=f"{ALIVE}(?: |$)(.*)"))     
async def iq(rekhsoevent):
    reply_to_id = await reply_id(rekhsoevent)
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    iqevent = await edit_or_reply(rekhsoevent, "**🇮🇶︙ جاري فحص السورس **")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "🇮🇶︙"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾  𓃠"
    rekhso_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/c6a84d3209402e5cd422d.mp4"
    tg_bot = Config.TG_BOT_USERNAME
    me = await rekhsoevent.client.get_me()
    ...
    caption = iqcaption.format(        ALIVE_TEXT=ALIVE_TEXT,
        ...
        tg_bot=tg_bot,    )
    if rekhso_IMG:
        CAT = [x for x in rekhso_IMG.split()]
        PIC = random.choice(CAT)
        try:
            await rekhsoevent.client.send_file(rekhsoevent.chat_id, PIC, caption=caption, reply_to=reply_to_id)
...
.🇮🇶 : version 7.5  𓇡.
.🇮🇶 : Source  : @pmmvn  𓇡."""
@rekhso.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/telethonAr/TelethonArab",)
@rekhso.on(admin_cmd(pattern="حساب كيثاب( -l(\d+))? ([\s\S]*)"))    
async def _(event):
...
@rekhso.on(admin_cmd(pattern="فارات تنصيبي(?: |$)(.*)"))    
async def _(event):
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (f"🇮🇶︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس ديو @pmmvn :**\n\n{o}")
    await edit_or_reply(event, OUTPUT)

if Config.PLUGIN_CHANNEL:

    async def install():
        documentss = await rekhso.get_messages(            Config.PLUGIN_CHANNEL, None, filter=InputMessagesFilterDocument        )
        ...
        for module in range(total):
            ...
            if os.path.exists(f"userbot/plugins/{plugin_name}"):
                return
            downloaded_file_name = await rekhso.download_media(                await rekhso.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),                "userbot/plugins/",            )
            ...
            while flag:
                try:
                    ...
                    if check > 5:
                        break
            if BOTLOG:
                await rekhso.send_message(                    BOTLOG_CHATID,                    f"**🇮🇶︙   تحـميل المـلف 🗂️  : `{os.path.basename(downloaded_file_name)}`  تـم بنجـاح ✔️**",                )

    rekhso.loop.create_task(install())
@rekhso.on(admin_cmd(pattern=f"{UPDATE}(?: |$)(.*)"))    
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**🇮🇶︙   تم تحديث سورس ديو ↻**")
    sandy = await edit_or_reply(event , "🇮🇶︙  جـاري تـحديـث  ســورس ديـــو  🔄\n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم" ,)
    try:
    ...
    try:
        delgvar("ipaddress")
        await rekhso.disconnect()
    except CancelledError:
        pass
    except Exception as e:
        LOGS1.error(e)
@rekhso.on(admin_cmd(pattern="مساعده(?:\s|$)([\s\S]*)"))
...
قناة السورس  : @pmmvn
 """)
@rekhso.on(admin_cmd(pattern="اطفاء مؤقت( [0-9]+)?$"))    
async def _(event):
...
@rekhso.on(admin_cmd(pattern="تاريخ التنصيب$"))
async def psu(event):
...
@rekhso.on(admin_cmd(pattern="(اضف|جلب|حذف) فار ([\s\S]*)"))    
async def bad(event):
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**🇮🇶︙  📑 يرجى متابع قناه السورس تجدها هنا : @pmmvn")
...
@rekhso.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
async def inlinerekhso(rekhso):
    ...
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"hsb1")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"hsb2")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"hsb3")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"hsb4")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"ord1hs")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑   اوامر الحساب   ⦒  :**"
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.on(admin_cmd(pattern="usage(?: |$)(.*)"))    
...
@rekhso.on(admin_cmd(pattern="(herokulogs|logs)(?: |$)(.*)"))    
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await edit_delete(dyno, "Set the required vars in heroku to function this normally `HEROKU_API_KEY` and `HEROKU_APP_NAME`.")
    try:
...
@rekhso.on(admin_cmd(pattern="استخدامي$"))
async def psu(event):
    uname = platform.uname()
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        cpuu = "**حجم استخدامك لديو :**\n"
    cpuu += f"الاستخدام : `{psutil.cpu_percent()}%`\n"
    svmem = psutil.virtual_memory()
    help_string = f"{str(cpuu)}\n"
    await event.edit(help_string)
@rekhso.on(admin_cmd(pattern="سرعه الانترنيت(?:\s|$)([\s\S]*)"))    
async def _(event):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlinerekhso(rekhso):
        builder = rekhso.builder
        result = None
        query = rekhso.text
        await bot.get_me()
        if query.startswith("تنصيب") and rekhso.query.user_id == bot.uid:
            buttons = [[Button.url("1- قناة السورس", "https://t.me/pmmvn"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@iklemdodo2020/stringsession-1#main.py"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/33xx/du"),],[Button.url("المطـورين 👨🏼‍💻", "https://t.me/REKHSO"),]]
            if rekhsoPC and rekhsoPC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(rekhsoPC, text=help1, buttons=buttons, link_preview=False)
            elif rekhsoPC:
                result = builder.document(rekhsoPC,title="rekhso",text=help1,buttons=buttons,link_preview=False)
            else:
                result = builder.article(title="rekhso",text=help1,buttons=buttons,link_preview=False)
            await rekhso.answer([result] if result else None)
@bot.on(admin_cmd(outgoing=True, pattern="تنصيب"))
async def reporekhso(rekhso):
    if rekhso.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
    if rekhso.reply_to_msg_id:
        await rekhso.get_reply_message()
    response = await bot.inline_query(TG_BOT, "تنصيب")
    await response[0].click(rekhso.chat_id)
    await rekhso.delete()
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"play1")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"play2")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"play3")))
@check_owner
async def inlinerekhso(rekhso):
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"ord1pl")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑   اوامر الالعاب   ⦒  :**"
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"shag1")))
@check_owner
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"shag2")))
@check_owner
async def inlinerekhso(rekhso):
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"ordsag1")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑   اوامر الصيغ   ⦒  :**"
    buttons = [[Button.inline("اوامر الصيغ  1", data="shag1"),],[Button.inline("اوامر الصيغ 2", data="shag2"),],[Button.inline("رجوع", data="orders"),]]
    await rekhso.edit(text, buttons=buttons)
@rekhso.on(admin_cmd(pattern=f"{ORDERS}(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, """✐  ⦗ اوامـر سـورس ديو ⦘
                                             ┉┉┉┉┉┉×┉┉┉┉┉
〖`.م1`〗⏎  اوامر الحساب 1
〖`.م2`〗⏎  اوامر الحساب 2
〖`.م3`〗⏎  اوامر الحساب 3
...
**✐ :  يعطيك رموز للزخرفه2 ❝**
⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮""")
@rekhso.on(admin_cmd(pattern="م27(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م28(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م29(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م30(?: |$)(.*)"))    
async def iq(event):
...
============================
  ⦑   شرح الاوامر : @pmmvn   ⦒""")
@rekhso.on(admin_cmd(pattern="م31(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"ordahln1")))
@check_owner
async def inlinerekhso(rekhso):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerekhso(rekhso):
        ...
        query = rekhso.text
        await bot.get_me()
        if query.startswith("اوامر الاعلانات(?: |$)(.*)") and rekhso.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الاعلانات", data="ordahln1"),]]
            result = builder.article(title="rekhso", text=help2, buttons=buttons, link_preview=False)
            await rekhso.answer([result] if result else None)
...
async def reporekhso(rekhso):
    ...
    await response[0].click(rekhso.chat_id)
    await rekhso.delete()
@rekhso.on(admin_cmd(pattern="م15(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م16(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م17(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م18(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م19(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م20(?: |$)(.*)"))    
async def iq(event):
...
async def inlinerekhso(rekhso):
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"ord1G")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑   اوامر الكروب   ⦒  :**"
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"G1")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑  اوامر الكروب 1     ⦒  :**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑴  ⦙ `.كتم + الرد ع الشخص`\n**✐ : يكتم الشخص من الخاص او الكروبات فقط اذا كانت عندك صلاحيه حذف رسائل ❝**\n \n⑵  ⦙ `. الغاء كتم + الرد ع الشخص`\n**✐ :  يجلب لك جميع معرفات المشرفين في الكروب  ❝**\n ⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑶  ⦙ `.البوتات`\n**✐ : يجلب لك جميع معرفات البوتات في الكروب ❝**\n \n⑷  ⦙ `.الأعضاء`\n**✐ : اضهار قائمة الاعضاء للكروب اذا هواي سيرسل ملف كامل لمعلوماتهم  ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑸  ⦙ `.معلومات`\n**✐ : سيرسل لك جميع معلومات الكروب بالتفصيل ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑹ ⦙ `.مسح المحظورين`\n**✐ : يمسح جميع المحظورين في الكروب ❝**\n ⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑺ ⦙ `.المحذوفين`\n**✐ : يجلب لك جميع الحسابات المحذوفه ❝**\n\n⑻ ⦙ `.المحذوفين تنظيف`\n**✐ : يمسح جميع الحسابات المحذوفه في الكروب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑼ ⦙ `.احصائيات الاعضاء`\n**✐ : يمسح جميع المحظورين في الكروب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑽ ⦙ `.انتحال + الرد ع الشخص`\n**✐ : يقوم بأنتحال الشخص ويضع صورته ونبذته واسمه في حسابك عدا المعرف ❝**\n\n⑾ ⦙ `.الغاء الانتحال + الرد ع الشخص`\n**✐ : يقوم بألغاء الانتحال وسيرجع معلومات المذكوره بالسورس ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n\n"
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.on(admin_cmd(pattern="تحميل الملف(?: |$)(.*)"))    
async def install(event):
    if event.reply_to_msg_id:
        try:
            ...
            os.remove(downloaded_file_name)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"G2")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑   اوامر الكروب 2   ⦒  :**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑴  ⦙  `.ترحيب + الرساله` \n**✐ : يضيف ترحيب في الكروب اي شخص ينضم راح يرحب بي  ❝**\n⑵  ⦙   `.مسح الترحيبات` \n**✐ :  ييقوم بمسح الترحيب من الكروب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n\n  ⦙  `.ترحيباتي` \n**✐ :  يضهر لك جميع الترحيبات التي وضعتها في الكروب ❝**\n⑷  ⦙ `.رساله الترحيب السابقه تشغيل`  \n**✐ :  عندما يحدث تكرار سيحذف رساله الترحيب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑸  ⦙  `.رساله الترحيب السابقه ايقاف`\n**✐ :  عندما يحدث تكرار لا يحذف رساله الترحيب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑹ ⦙  `.اضف رد + الكلمه` \n**✐ :  مثلاً تدز رساله هلو تسوي عليها رد بهلوات ❝**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑺ ⦙ `.مسح رد + الكلمه` \n**✐ :  سيحذف الكلمه الي انت ضفتها ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n\n⑻ ⦙  `.جميع الردود` \n **✐ :  يجلب لك جميع الردود الذي قمت بأضافتها  ❝**\n⑼ ⦙  `.مسح جميع الردود` \n**✐ :  يمسح جميع الردود الي انت ضفتها ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑽ ⦙  `.صنع مجموعه + اسم المجموعه`\n**✐ : يقوم بعمل مجموعه خارقه ❝**\n \n⑾ ⦙  `.صنع قناه +  اسم القناة`\n**✐ : يقوم بعمل قناه خاصه  ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑿ ⦙ `.عدد رسائلي`\n**✐ : سيظهر لك عدد رسائلك في الكروب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n\n"
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"G3")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑   اوامر الكروب 3   ⦒  :**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑴  ⦙  `.تفعيل حمايه المجموعه`\n**✐ : يقوم غلق جميع صلاحيات المجموعه يبقي فقط ارسال  الرسائل❝**\n \n⑵  ⦙ `تعطيل حمايه المجموعه`\n**✐ :  يقوم بتشغيل جميع صلاحيات المجموعة ماعدا تغير المعلومات و التثبيت و اضافه اعضاء تبقى مسدوده❝**\n\n⑶  ⦙ `.صلاحيات المجموعه`\n**✐ : يقوم بعرض صلاحيات المجموعه المغلقه والمفتوحه❝**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n⑷  ⦙  `.رفع مشرف + الرد على شخص`\n**✐ : يرفع الشخص مشرف يعطي صلاحيه حذف رسائل والتثبيت فقط❝**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑸  ⦙ `.منع + كلمة`\n**✐ : منع كلمه من الارسال في الكروب**❝\n⑹ ⦙ `.الغاء منع + كلمه`\n**✐ : يقوم بالغاء منع الكلمه ❝** \n⑺ ⦙ `.قائمه المنع`\n**✐ : يقوم بجلب جميع الكلمات الممنوعه في الكروب ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑻ ⦙ ` .تاك + ( الاعداد المحدده وثابتة فقط) ⤵️`\n  ( 10 - 50 - 100 - 200  )\n**✐ : يجلب لك الاعضاء بالروابط بالعدد المحدد ❝**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑼ ⦙ `.معرفات + ( الاعداد المحدده وثابتة فقط) ⤵️`\n  ( 10 - 50 - 100 - 200  )\n**✐ :جلب لك معرفات الاعضاء بالعدد المحدد ❝**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮\n"
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.on(admin_cmd(pattern="مسح الملف(?: |$)(.*)"))    
...
@rekhso.on(admin_cmd(pattern="هاش ([\s\S]*)"))    
...
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"G4")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑  اوامر الكروب 4     ⦒  :**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑴  ⦙ `.تنظيف الوسائط` \n ✐: ينضف جميع ميديا من صور وفديوهات و متحركات** او ( `.تنظيف الوسائط + العدد`) ** \n⑵  ⦙ `.حذف الرسائل`\n**✐ :  يحذف جميع الرسائل بلكروب ** \n ` او  `.حذف الرسائل + العدد \n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑶  ⦙ `.مسح + الرد على رسالة`\n**✐ :  يحذف الرساله الي راد عليها فقط **\n⑷  ⦙ `.غادر + بلكروب دزها`\n**✐ :  يغادر من المجموعه او من القناة**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑸  ⦙ ` .تفليش`\n**✐ :  يطرد جميع الي بلكروب الامر صار احسن ومتطور واسرع**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑹  ⦙ `.اضافه + رابط الكروب `\n**✐ :  يضيفلك جميع الاعضاء الي برابط الكروب يضيفهم بكروبك ( يجب ان تتاكد انو مامحضور حسابك ارسل ⬅️( .فحص الحظر ) علمود تتاكد حسابك محظور او لا) \n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑺  ⦙ `.جلب الوقتيه + الرد على الصورة`\n**✐ :  الرد على صوره سريه وقتيه سوف يتم تحويلها الى رسائل المحفوضه كصورة عادية\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⑻  ⦙ `.تاك بالكلام + الكلمه + معرف الشخص`\n**✐ :  يسوي تاك للشخص بالرابط جربه وتعرف**\n⑼  ⦙ `.نسخ + الرد على رساله`\n**✐ :  يرسل الرساله التي رديت عليها **\n⑽  ⦙ `.ابلاغ الادمنيه`\n**✐ :  يسوي تاك لجميع الادمنيه ارسله هذا الامر بلمجموعه في حاله اكو تفليش او مشكلة**\n⑾  ⦙ `.المشرفين` \n**✐ : يجيب الك جميع المشرفين في المجموعه او القناه**\n⑿  ⦙ `.البوتات` \n**✐ :  يجيب الك جميع بوتات في المجموعه او قناه**"
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.tgbot.on(CallbackQuery(data=re.compile(rb"G5")))
@check_owner
async def inlinerekhso(rekhso):
    text = "**⚡  ⦑  اوامر الكروب 5     ⦒  :**\n\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑴  ⦙ `.تحذير التكرار + عدد رسائل`\n**✐ :  اي شخص بلكروب يكرر رسائل مالته بلعدد المحدد يقيدة مهما كان رتبته**\n ⑵  ⦙ ` .تحذير تكرار 99999 `\n✐ :  هذا الامر ستعمله من تريد تلغي التحذير لان مستحيل احد يكرر هل عدد ف اعتبار ينل(غي التحذير**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑶  ⦙ ` .حظر + الرد على شخص`\n✐ : حظر الشخص من المجموعه او الكروب**\n ⑷  ⦙ ` .الغاء الحظر + الرد على شخص`\n✐ :  يلغي حظر الشخص من المجموعه او الكروب**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑸  ⦙ ` .بدء مكالمه `\n✐ :  يقوم بتشغيل مكالمه في المجموعه**\n ⑹  ⦙ `.دعوه للمكالمه`\n✐ : يتم دعوه الاعضاء للمكالمة الشغاله**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⑺  ⦙ ` .تنزيل مشرف + الرد على شخص`\n✐ :  يقوم بازاله الشخص من الاشراف **\n ⑻  ⦙ ` .تثبيت + الرد على رساله`\n✐ : شرح : تثبيت الرساله التي رديت عليها**⒀  ⦙ `.الأعضاء`\n**✐ :  اضهار قائمة الاعضاء للمجموعة اذا هواي يرسلك ملف كامل لمعلوماتهم**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n⒁  ⦙ `.تفليش `\n**✐ :  يقوم بأزاله جميع اعضاء المجموعه او القناة الى 0**\n⤪⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⟿⤮ \n ⒂  ⦙ `.مسح المحظورين`\n**✐ :  يمسح جميع المحظورين في المجموعه او القناه **\n⒃  ⦙ `.المحذوفين`\n**✐:  يجلب لك جميع الحسابات المحذوفه في المجموعه او القناه**\n⒄  ⦙ `.المحذوفين تنظيف`\n**✐ :  مسح جميع الحسابات المحذوفه في المجموعه او القناة**\n⒅  ⦙ `.احصائيات الاعضاء`\n**✐ :  يرسل اليك جميع معلومات اعضاء المجموعه منها عدد الحسابات المحذوفه او الحسابات النشطه او الحسابات اخر ضهور وجميعهم**\n⒆  ⦙ `.عدد رسائلي`\n**✐ : يقوم بحساب عدد رسائلك في المجموعه او القناة**\n⒇  ⦙ `.جلب الاحداث`\n**✐ :  يرسل اليك اخر 20 رساله محذوفه في المجموعة من الاحداث**"
    ...
    await rekhso.edit(text, buttons=buttons)
@rekhso.on(admin_cmd(pattern="هاش(ين|دي) ([\s\S]*)"))    
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerekhso(rekhso):
        ...
        query = rekhso.text
        await bot.get_me()
        if query.startswith("اوامر الكروب(?: |$)(.*)") and rekhso.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlinerekhso(rekhso):
        ...
        query = rekhso.text
        await bot.get_me()
        if query.startswith("(الاوامر|الأوامر)") and rekhso.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر السورس", data="order1"), Button.inline("اوامر الحساب", data="ord1hs"),],[Button.inline("اوامر الكروب", data="ord1G"), Button.inline("اوامر الالعاب", data="ord1pl"),],[Button.inline("اوامر الصيغ", data="ordsag1"), Button.inline("اوامر الاغاني", data="ordSONG"),], [Button.inline("اسم وقتي", data="order13"), Button.inline("اوامر الاعلانات", data="ordahln1"),],[Button.inline("اوامر التسليه", data="order14"),],[Button.inline("الفارات", data="ordvars"),]]
            result = builder.article(title="rekhso",text=help2,buttons=buttons,link_preview=False)
            await rekhso.answer([result] if result else None)
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerekhso(rekhso):
        ...
        query = rekhso.text
        await bot.get_me()
        if query.startswith("اوامر الحساب(?: |$)(.*)") and rekhso.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerekhso(rekhso):
        ...
        query = rekhso.text
        await bot.get_me()
        if query.startswith("اوامر الالعاب(?: |$)(.*)") and rekhso.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerekhso(rekhso):
        ...
        query = rekhso.text
        await bot.get_me()
        if query.startswith("اوامر الصيغ(?: |$)(.*)") and rekhso.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الصيغ", data="ordsag1"),]]
            result = builder.article(title="rekhso", text=help2, buttons=buttons, link_preview=False)
            await rekhso.answer([result] if result else None)
@rekhso.on(admin_cmd(pattern="م21(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م22(?: |$)(.*)"))    
async def iq(event):
...
@rekhso.on(admin_cmd(pattern="م23(?: |$)(.*)"))    
async def iq(event):
...
async def reporekhso(rekhso):
    ...
    await response[0].click(rekhso.chat_id)
    await rekhso.delete()
@rekhso.on(admin_cmd(pattern="فتح همسه(?: |$)(.*)"))    
async def iq(event):
